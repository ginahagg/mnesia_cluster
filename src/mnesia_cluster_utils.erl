...
join_cluster(DiscoveryNode, NodeType) ->
    ...
    case me_in_nodes(ClusterNodes) of
        false ->
            ...
            %% Join the cluster
            error_logger:info_msg("Clustering with ~p as ~p node~n",
                            [ClusterNodes, NodeType]),
            ok = init_db_with_mnesia(ClusterNodes, NodeType, true, true),
            mnesia_cluster_monitor:notify_joined_cluster(),
            ok;
        true ->
            error_logger:info_msg("Already member of cluster: ~p~n", [ClusterNodes]),
            {ok, already_member}
    end.
...
reset() ->
    ensure_mnesia_not_running(),
    error_logger:info_msg("Resetting Cluster~n", []),
    reset_gracefully().

force_reset() ->
    ensure_mnesia_not_running(),
    error_logger:info_msg("Resetting Cluster forcefully~n", []),
    wipe().
...
reset_gracefully() ->
    ...
    leave_cluster(),
    ensure_ok(mnesia:delete_schema([node()]), cannot_delete_schema),
    wipe().
...
wipe() ->
    ...
    ok = mnesia_cluster_file:recursive_delete(filelib:wildcard(dir() ++ "/*")),
    ok = mnesia_cluster_monitor:reset_cluster_status(),
    ok.
...
update_cluster_nodes(DiscoveryNode) ->
    ...
    case me_in_nodes(AllNodes) of
        true ->
            ...
            error_logger:info_msg("Updating cluster nodes from ~p~n",
                            [DiscoveryNode]),
...
forget_cluster_node(Node, RemoveWhenOffline) ->
    ...
    case {RemoveWhenOffline, is_running()} of
        ...
        {false,  true} -> error_logger:info_msg(
    ...
    case {mnesia:system_info(running_db_nodes) -- [Node], node_type()} of
        {[], disc} ->
            ...
            try
                ...
                mnesia_cluster_table:wait_for_replicated(),
        ...
        case is_running() of
            true  -> RunningNodes = cluster_nodes(running),
                     [{running_nodes, RunningNodes},
                      {cluster_name,  mnesia_cluster_nodes:cluster_name()},
                      {partitions,    mnesia_partitions(RunningNodes)}];
            false -> []
        end.

mnesia_partitions(Nodes) ->
    Replies = mnesia_cluster_monitor:partitions(Nodes),
...
cluster_status_from_mnesia() ->
    case is_running() of
        ...
        true ->
            ...
            case mnesia_cluster_table:is_present() of
        ...
        case cluster_status_from_mnesia() of
            ...
            {error, _Reason} ->
                {AllNodes0, DiscNodes0, RunningNodes0} =
                    mnesia_cluster_monitor:read_cluster_status(),
...
node_info() ->
    {erlang:system_info(otp_release), version(),
     cluster_status_from_mnesia()}.
...
ensure_schema_integrity() ->
    case mnesia_cluster_table:check_schema_integrity() of
        ok ->
            ok;
        {error, Reason} ->
...
copy_db(Destination) ->
    ...
    mnesia_cluster_file:recursive_copy(dir(), Destination).

force_load_filename() ->
    filename:join(dir(), "force_load").

force_load_next_boot() ->
    mnesia_cluster_file:write_file(force_load_filename(), <<"">>).
...
check_cluster_consistency(Node) ->
    case rpc:call(Node, mnesia_cluster_utils, node_info, []) of
        {badrpc, _Reason} ->
            {error, not_found};
        {_OTP, _MnesiaCluster, {error, _}} ->
            {error, not_found};
        {OTP, MnesiaCluster, {ok, Status}} ->
            case check_consistency(OTP, MnesiaCluster, Node, Status) of
                {error, _} = E -> E;
                {ok, Res}      -> {ok, Res}
            end;
        {_OTP, MnesiaCluster, _Hash, _Status} ->
            %% delegate hash checking implies version mismatch
            {ok, VSN} = application:get_key(mnesia_cluster, vsn),
            version_error("MnesiaCluster", VSN, MnesiaCluster)
    end.

%%--------------------------------------------------------------------
%% Hooks for `mnesia_cluster_monitor'
%%--------------------------------------------------------------------

on_node_up(Node) ->
    case running_disc_nodes() of
        [Node] -> error_logger:info_msg("cluster contains disc nodes again~n");
        _      -> ok
    end.

on_node_down(_Node) ->
    case running_disc_nodes() of
        [] -> error_logger:info_msg("only running disc node went down~n");
        _  -> ok
    end.
...
discover_cluster0(Node) ->
    rpc:call(Node, mnesia_cluster_utils, cluster_status_from_mnesia, []).
...
create_schema() ->
    ...
    start_mnesia(),
    ok = mnesia_cluster_table:create(),
    ensure_schema_integrity().
...
move_db() ->
    ...
    BackupDir = format("~s_~w~2..0w~2..0w~2..0w~2..0w~2..0w",
                  [MnesiaDir, Year, Month, Day, Hour, Minute, Second]),
    case mnesia_cluster_file:rename(MnesiaDir, BackupDir) of
        ok ->
    ...
    case is_running() of
        ...
        true ->
            ...
            case mnesia:del_table_copy(schema, Node) of
                {atomic, ok} ->
                    %% FIXME: MAYBE_HOOK
                    mnesia_cluster_monitor:notify_left_cluster(Node),
                    ok;
                {aborted, Reason} ->
...
leave_cluster(Node) ->
    case rpc:call(Node,
                  mnesia_cluster_utils, remove_node_if_mnesia_running, [node()]) of
        ok                          -> true;
...
check_consistency(OTP, MnesiaCluster) ->
    sequence_error(
      [check_otp_consistency(OTP),
       check_mnesia_cluster_consistency(MnesiaCluster)]).

check_consistency(OTP, MnesiaCluster, Node, Status) ->
    sequence_error(
      [check_otp_consistency(OTP),
       check_mnesia_cluster_consistency(MnesiaCluster),
    ...
    case me_in_nodes(RemoteAllNodes) of
        ...
        false ->
            {error, {inconsistent_cluster,
                     format("Node ~p thinks it's clustered "
...
version_error(Name, This, Remote) ->
    {error, {inconsistent_cluster,
             format("~s version mismatch: local node is ~s, "
...
check_mnesia_cluster_consistency(Remote) ->
    check_version_consistency(
      version(), Remote, "MnesiaCluster",
      fun version_minor_equivalent/2).
...
%% `mnesia_cluster_monitor:prepare_cluster_status_file/0'.
is_virgin_node() ->
    case mnesia_cluster_file:list_dir(dir()) of
        {error, enoent} ->
            true;
        {ok, []} ->
            true;
        {ok, [File1, File2]} ->
            lists:usort([dir() ++ "/" ++ File1, dir() ++ "/" ++ File2]) =:=
                lists:usort([mnesia_cluster_monitor:cluster_status_filename(),
                             mnesia_cluster_monitor:running_nodes_filename()]);
        {ok, _} ->
            false
    end.
...
find_auto_cluster_node([Node | Nodes]) ->
    ...
    case rpc:call(Node, mnesia_cluster_utils, node_info, []) of
        {badrpc, _} = Reason     -> Diag = mnesia_cluster_nodes:diagnostics([Node]),
                                    Fail("~p~n~s~n", [Reason, Diag]);
        %% old delegate hash check
        {_OTP, MnesiaCluster, _Hash, _} -> Fail("version ~s~n", [MnesiaCluster]);
        {OTP, MnesiaCluster, _}         -> case check_consistency(OTP, MnesiaCluster) of
...
error_description(offline_node_no_offline_flag) ->
    "You are trying to remove a node from an offline node. That is dangerous, "
        "but can be done with the --offline flag.";
