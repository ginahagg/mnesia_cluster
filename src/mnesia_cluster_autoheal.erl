...
-module(mnesia_cluster_autoheal).

-export([init/0, maybe_start/1, app_node_down/2, node_down/2, handle_msg/3]).

%% The named process we are running in.
-define(SERVER, mnesia_cluster_monitor).
...
%% determines that a node has stopped just by seeing if its defined app
...
%%   autoheal_safe_to_start message to the mnesia_cluster_outside_app_process
%%   instead.
...
maybe_start(not_healing) ->
    case enabled() of
        true  -> [Leader | _] = lists:usort(mnesia_cluster_utils:cluster_nodes(all)),
                 send(Leader, {request_start, node()}),
                 error_logger:info_msg("Autoheal request sent to ~p~n", [Leader]),
                 not_healing;
        false -> not_healing
    end;
maybe_start(State) ->
...
enabled() ->
    {ok, autoheal} =:= application:get_env(mnesia_cluster, cluster_partition_handling).


%% This is the winner receiving its last notification that a node has
%% stopped - all nodes can now start again
app_node_down(Node, {winner_waiting, [Node], Notify}) ->
    error_logger:info_msg("Autoheal: final node has stopped, starting...~n",[]),
    notify_safe(Notify),
    not_healing;

app_node_down(Node, {winner_waiting, WaitFor, Notify}) ->
    {winner_waiting, WaitFor -- [Node], Notify};

app_node_down(Node, {leader_waiting, [Node]}) ->
    not_healing;

app_node_down(Node, {leader_waiting, WaitFor}) ->
    {leader_waiting, WaitFor -- [Node]};

app_node_down(_Node, State) ->
...
node_down(Node, {winner_waiting, _, Notify}) ->
    error_logger:info_msg("Autoheal: aborting - ~p went down~n", [Node]),
...
handle_msg({request_start, Node},
    ...
    error_logger:info_msg("Autoheal request received from ~p~n", [Node]),
    mnesia_cluster_monitor:ping_all(),
    case mnesia_cluster_monitor:all_app_nodes_up() of
        false -> not_healing;
        true  -> AllPartitions = all_partitions(Partitions),
                 {Winner, Losers} = make_decision(AllPartitions),
                 error_logger:info_msg("Autoheal decision~n"
                                 "  * Partitions: ~p~n"
                                 "  * Winner:     ~p~n"
                                 "  * Losers:     ~p~n",
...
handle_msg({request_start, Node},
           State, _Partitions) ->
    error_logger:info_msg("Autoheal request received from ~p when in state ~p; "
                    "ignoring~n", [Node, State]),
    State;

handle_msg({become_winner, Losers},
           not_healing, _Partitions) ->
    error_logger:info_msg("Autoheal: I am the winner, waiting for ~p to stop~n",
                    [Losers]),
    {winner_waiting, Losers, Losers};

handle_msg({become_winner, Losers},
           {winner_waiting, WaitFor, Notify}, _Partitions) ->
    error_logger:info_msg("Autoheal: I am the winner, waiting additionally for "
                    "~p to stop~n", [Losers]),
...
handle_msg({winner_is, Winner},
    ...
    mnesia_cluster_monitor:run_outside_applications(
      fun () ->
...
notify_safe(Notify) ->
    [{mnesia_cluster_outside_app_process, N} ! autoheal_safe_to_start || N <- Notify].
...
all_partitions(PartitionedWith) ->
    ...
    Partitions = [{node(), PartitionedWith} |
                  mnesia_cluster_monitor:partitions(Nodes -- [node()])],
